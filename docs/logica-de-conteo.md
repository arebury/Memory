...
En la tabla, las filas `deleted` se renderizan con opacidad reducida (~60%) y tooltip explicativo del motivo (por defecto *"Custodia GDPR vencida · datos no recuperables"* — el copy puede variar según el motivo concreto de la restricción). El supervisor las ve marcadas pero NO se le restringe la interacción (decisión 15.45 con PM): el checkbox sigue activo y el icono de estado abre el reproductor normalmente, para que pueda revisar lo que SÍ esté disponible y descargar formatos compatibles desde el modal Download del legacy (que gestiona el caso "no descargable" con su propio aviso). Lo único que el sistema sí bloquea silenciosamente es la inclusión en bulks de transcripción/análisis: `BulkTranscriptionModal` las filtra antes de calcular contadores. El defensive filter en `handleRequestTranscription` las descarta también si llegan por otra vía.
...
- **Retención vencida** (ejemplo canónico: custodia GDPR). Fila atenuada (opacidad ~60%), tooltip explicando el motivo de la restricción al hover. Checkbox seleccionable y `handleRowClick` abre el reproductor normalmente — el supervisor puede revisar lo que SÍ está disponible y descargar formatos compatibles desde el modal Download del legacy. La única restricción es que el bulk de transcripción/análisis las excluye silenciosamente del lote (no procesables). El ejemplo del tooltip es GDPR, pero la regla se aplica a cualquier restricción legal de retención que afecte al chat.
